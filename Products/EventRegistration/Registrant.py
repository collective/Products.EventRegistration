...
from Products.EventRegistration.config import HONORIFICS
...
class Registrant(BaseContent):
	...
	def confirmToRegistrant(self, request):
		...
		body = ''
		for line in propsheet.message_to_registrant:
			line = str(line) % messagevars
			body += textwrap.fill(line, 72) + '\n'
